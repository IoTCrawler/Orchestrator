  ...
  stages{
      stage('Copying2'){   
        steps{
          ...
          sh 'cp -R /mnt/share/orchestrator /root/orchestrator > /dev/null 2>&1'
          sh 'chmod -R 777 /root/orchestrator'
        }
      }
      stage('Build') {
          steps{
             ...
             withCredentials([usernamePassword(credentialsId: 'b3a56de1-d06e-45c8-9ce4-5cc1ddb0cbb8', usernameVariable: 'CI_REGISTRY_USER', passwordVariable: 'CI_REGISTRY_PASSWORD')])     {
               ...
               //sh 'cd /root/orchestrator/orchestrator && make build-image'
            }                 
      ...
      stage('StartContainers') {
          steps{
             sh 'cd /root/orchestrator/orchestrator && make start-containers'  
             sh 'sleep 10'
          }
      }  
      stage('Tests') {
          steps{
             //sh 'cd /root/orchestrator/orchestrator && make rpc-client-test'
            //sh 'sleep 3600'
      ...
      stage('StopContainers') {
          steps{
             sh 'cd /root/orchestrator/orchestrator && make stop-containers'  
          }
      }  
      //stage('Results') {
      //    steps{
