...
if [ "$1" = "install" ]; then
   echo "#Fiware/models: checking ngsi2 dependency"
	(if [ ! -d ~/.m2/repository/com/orange/fiware/ngsi2-client ] || [ -n "$REBUILD_ALL" ]; then CURR=$(pwd) && sh make.sh prepare-ngsi2-api && cd $CURR; fi);

  echo "# Fiware-models: checking com/agtinternational/iotcrawler"
	(if [ ! -d ~/.m2/repository/com/agtinternational/iotcrawler ] || [ -n "$REBUILD_ALL" ]; then CURR=$(pwd) && cd "../IoTCrawler" && mvn install && cd $CURR; fi);
