...
if [ "$1" = "install" ]; then
  ...
  echo "# Main: checking com/agtinternational/iotcrawler"
	(if [ ! -d ~/.m2/repository/com/agtinternational/iotcrawler ] || [ -n "$REBUILD_ALL" ]; then cd "IoTCrawler" && mvn install && cd ..; fi);
	echo "# Main: checking com/agtinternational/iotcrawler/fiware-models"
	(if [ ! -d ~/.m2/repository/com/agtinternational/iotcrawler/fiware-models ] || [ -n "$REBUILD_ALL" ]; then cd "com.agtinternational.iotcrawler.fiware-models" && sh make.sh install && cd ..; fi);
	echo "# Main: checking com/agtinternational/iotcrawler/fiware-clients"
	(if [ ! -d ~/.m2/repository/com/agtinternational/iotcrawler/fiware-clients ] || [ -n "$REBUILD_ALL" ]; then cd "com.agtinternational.iotcrawler.fiware-clients" && sh make.sh install && cd ..; fi);
  echo "# Main: checking com/agtinternational/iotcrawler/core"
	(if [ ! -d ~/.m2/repository/com/agtinternational/iotcrawler/core ] || [ -n "$REBUILD_ALL" ]; then cd "com.agtinternational.iotcrawler.core" && mvn install -DskipTests=true && cd ..; fi);
